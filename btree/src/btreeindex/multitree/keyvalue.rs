...
use crate::btreeindex::backtrack::UpdateBacktrack;
...
use crate::btreeindex::{tree_algorithm, Node, PageId, Pages, StaticSettings};
...
impl<'a, 'b, Tag: FixedSize, K: FixedSize, V: FixedSize> WriteTx<'a, 'b, Tag, K, V> {
    ...
    pub fn update(&mut self, key: &K, f: impl Fn(&V) -> V) -> Result<(), BTreeStoreError> {
        UpdateBacktrack::new_search_for(&mut self.tx, key).update(f)?;
    ...
    pub fn commit(self) -> Result<ReadTx<K>, BTreeStoreError> {
        let delta = match self.tx.commit::<K>() {
